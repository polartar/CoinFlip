		...
		if ($results && $results1) {
			...
			if($error == 0) {
				try {
					...
					$mail->Subject = 'Account Confirmation - BTCBattles.com';
					$mail->Body    = 'Access this link to confirm your account: <br> http://139.180.222.171/confirm.php?code='.$code;
					$mail->AltBody = 'BTCBattles agrements, automatic email!';

					$mail->send();

					$exec = $db->query('INSERT INTO `users` SET `email` = '.$db->quote($email).', `password` = '.$db->quote(md5($password)).', `username` = '.$db->quote($username).', `verify_code` = '.$db->quote($code));
					$exec->fetch(PDO::FETCH_ASSOC);

					$error_message = 'Your account is registered, confirm with a link sent to your email';
